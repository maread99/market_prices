...
class TestStartEnd:
    ...
    def test_start_ok_end_rorl(
        ...
        match = re.escape(
            "`end` cannot evaluate to a later date than the latest date for which prices are available.\nThe latest date for which prices are available for interval '1 day, 0:00:00' is 2022-06-15, although `end` evaluates to 2022-06-16."
        )
...
class TestDuration:
    ...
    def test_defined_ool_evaluated_ok(
        ...
        match_daily = re.escape(
            "`end` cannot evaluate to a later date than the latest date for which prices are available.\nThe latest date for which prices are available for interval '1 day, 0:00:00' is 2022-06-15, although `end` evaluates to 2022-07-13."
        )
        match_mo = re.escape(
            "`end` cannot evaluate to a later date than the latest date for which prices are available.\nThe latest date for which prices are available for interval 'DOInterval.M1' is 2022-05-31, although `end` evaluates to 2022-06-30."
        )
...
class TestOutsideLimits:
    ...
    def test_period_defined_to_right_of_available(
        ...
        for intrvl, priority, strict in itertools.product(
            ...
            match = re.escape(
                "`start` cannot be a later date than the latest date for which prices are available.\nThe latest date for which prices are available for interval '1 day, 0:00:00' is 2022-06-15, although `start` received as 2022-07-13."
            )
